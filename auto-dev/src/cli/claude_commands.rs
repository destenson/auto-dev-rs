...
use auto_dev_core::claude::{
    ClaudeCommand, ClaudeConfigDiscovery, CommandExecutor, CommandOutput, CommandParser,
    CommandRegistrySystem, CommandSource,
};
...
impl ClaudeCommandHandler {
    ...
    pub async fn new() -> Result<Self> {
        ...
        Ok(Self { registry, discovery, commands_cache })
    }
            ...
            for arg in &cmd.arguments {
                ...
                match arg.arg_type.as_deref() {
                    Some("boolean") | Some("bool") => {
                        clap_arg = clap_arg.action(ArgAction::SetTrue).num_args(0);
                    }
                    Some("array") | Some("list") => {
                        clap_arg = clap_arg.action(ArgAction::Append).num_args(1..);
                    }
                    _ => {
                        clap_arg = clap_arg.action(ArgAction::Set).num_args(1);
                    }
                }
    ...
    pub async fn execute_command(&self, name: &str, args: &ArgMatches) -> Result<()> {
        let cmd = self
            .commands_cache
            .get(name)
...
pub async fn execute_claude_command(matches: &ArgMatches) -> Result<()> {
    ...
    let (name, sub_matches) =
        matches.subcommand().ok_or_else(|| anyhow::anyhow!("No Claude subcommand specified"))?;
...
pub async fn handle_claude_command(command: String, args: Vec<String>) -> Result<()> {
    ...
    let cmd = handler.get_command(&command).ok_or_else(|| {
        let available = handler.list_command_names();
        if available.is_empty() {
            anyhow::anyhow!(
                "No Claude commands available. Create command files in .claude/commands/"
            )
        } else {
    ...
    let mut cmd_app = Command::new(command.clone()).about(cmd.description.clone());

    // Add argument definitions
    for arg in cmd.arguments {
        let mut clap_arg =
            Arg::new(clap::builder::Str::from(arg.name.clone())).help(arg.description.clone());
    ...
    let matches =
        cmd_app.try_get_matches_from(std::iter::once(command.clone()).chain(args.into_iter()))?;
