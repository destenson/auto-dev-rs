...
/// Execute the dogfood command
pub async fn execute(cmd: DogfoodCommand) -> Result<()> {
    match cmd.subcommand {
        DogfoodSubcommand::Validate => validate_config().await,
        DogfoodSubcommand::Monitor { watch } => monitor_source(watch).await,
        DogfoodSubcommand::CheckSafety => check_safety().await,
        DogfoodSubcommand::Run { task, skip_validation } => {
            run_dogfood(task, cmd.dry_run, skip_validation).await
        },
        DogfoodSubcommand::Init => init_dogfood().await,
        DogfoodSubcommand::Status => show_status().await,
    }
}
...
async fn validate_config() -> Result<()> {
    ...
    let config_content = fs::read_to_string(&config_path)
        .context("Failed to read dogfood configuration")?;
    
    let config: DogfoodConfig = toml::from_str(&config_content)
        .context("Failed to parse dogfood configuration")?;
    ...
    for allow in &config.synthesis.allow_paths {
        for deny in &config.synthesis.deny_paths {
            if allow == deny || allow.starts_with(deny) {
                return Err(anyhow::anyhow!(
                    "Conflicting paths: '{}' is both allowed and denied", allow
                ));
            }
        }
    }
...
async fn monitor_source(watch: bool) -> Result<()> {
    ...
    if watch {
        ...
        // Show TODOs found
        let todo_reqs: Vec<_> = specs.iter()
