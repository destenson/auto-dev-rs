...
use auto_dev_core::docs::{
    ChangelogBuilder, ChangelogConfig, DocConfig, DocFormat, DocumentationSystem,
};
...
async fn generate_docs(args: DocsArgs) -> Result<()> {
    ...
    let result = doc_system.generate_all().await.context("Failed to generate documentation")?;
...
async fn check_docs(args: DocsArgs) -> Result<()> {
    ...
    let issues = doc_system.validate().await.context("Failed to validate documentation")?;
...
async fn check_coverage(args: DocsArgs) -> Result<()> {
    ...
    let coverage = doc_system.check_coverage().await.context("Failed to check coverage")?;
...
async fn update_changelog(args: DocsArgs) -> Result<()> {
    ...
    changelog.load().context("Failed to load existing changelog")?;

    // Extract changes from git
    changelog.extract_from_git(None).await.context("Failed to extract changes from git")?;

    // Update the file
    changelog.update().context("Failed to update changelog")?;
...
async fn generate_api_docs(args: DocsArgs) -> Result<()> {
    ...
    let files = doc_system.generate_api_docs().await.context("Failed to generate API docs")?;
...
async fn update_readme(args: DocsArgs) -> Result<()> {
    ...
    doc_system.update_readme().await.context("Failed to update README")?;
