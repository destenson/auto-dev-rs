...
use auto_dev_core::validation::{
    ValidationConfig, ValidationPipeline, SpecValidator, ToolRegistry,
    GeneratedCode
};
...
pub async fn execute(args: ValidateArgs) -> Result<()> {
    match args.command {
        Some(ValidateCommands::Spec { spec }) => {
            validate_specification(&args.path, &spec).await
        }
        Some(ValidateCommands::Discover) => {
            discover_tools().await
        }
...
async fn check_essential_tools(project_path: &Path) -> Result<()> {
    ...
    if !missing_tools.is_empty() {
        ...
        match input.trim() {
            ...
            _ => {
                return Err(anyhow::anyhow!("Required tools missing. Cannot proceed with validation."));
            }
        }
    }
...
fn generate_full_report(result: &auto_dev_core::validation::ValidationResult) -> String {
    ...
    if !result.errors.is_empty() {
        ...
        for (i, error) in result.errors.iter().enumerate() {
            ...
            if let Some(location) = &error.location {
                report.push_str(&format!("- **Location:** {}:{}\n", location.file, location.line.unwrap_or(0)));
            }
            if let Some(fix) = &error.fix_suggestion {
