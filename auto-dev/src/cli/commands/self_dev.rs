...
async fn set_change_limit(limit: usize) -> Result<()> {
    ...
    if let Some(orchestrator) = orchestrator_lock.as_ref() {
        orchestrator
            .handle_control_command(ControlCommand::SetMaxChangesPerDay(limit))
            .await?;
...
fn parse_safety(safety: &str) -> Result<SafetyLevel> {
    match safety.to_lowercase().as_str() {
        ...
        _ => Err(anyhow::anyhow!(
            "Invalid safety level. Use: permissive, standard, or strict"
        )),
    }
}

async fn run_manual_task(task: String, dry_run: bool, skip_validation: bool, no_confirm: bool) -> Result<()> {
...
async fn validate_configuration() -> Result<()> {
    ...
    println!("📦 Project: {} v{}", self_target_config.project.name, self_target_config.project.version);
    ...
    for watch_path in &self_target_config.monitor.watch {
        for exclude_path in &self_target_config.monitor.exclude {
            if watch_path.starts_with(exclude_path) {
                println!("Conflict: '{}' is watched but excluded by '{}'", watch_path, exclude_path);
            }
        }
    }
...
async fn check_safety_boundaries() -> Result<()> {
    ...
    for (path, desc) in critical_paths {
        let protected = self_target_config.safety.forbidden_paths
            .iter()
