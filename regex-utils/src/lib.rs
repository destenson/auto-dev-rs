...
pub mod project_name {
    ...
    pub static NAME_PATTERN: Lazy<Regex> =
        Lazy::new(|| Regex::new(r"(?:project|name):\s*(\w+)").expect("Invalid regex pattern"));
    ...
    impl LanguageMatcher {
        ...
        pub fn detect(&self, text: &str) -> Option<String> {
            ...
            for (pattern, name) in &self.patterns {
                if Regex::new(pattern).ok().and_then(|re| re.find(&text_lower)).is_some() {
...
mod tests {
    ...
    fn test_project_name_extraction() {
        ...
        assert_eq!(project_name::extract("project: myproject"), Some("myproject".to_string()));
    }
    ...
    fn test_language_detection() {
        ...
        assert_eq!(matcher.detect("build a rust application"), Some("Rust".to_string()));

        assert_eq!(matcher.detect("create a java app"), Some("Java".to_string()));

        assert_eq!(matcher.detect("javascript project"), Some("JavaScript".to_string()));
    }
}
