...
impl UnitTestStrategy {
    pub fn new() -> Self {
        Self { include_happy_path: true, include_error_cases: true, include_boundary_values: true }
    }
    ...
    fn generate_happy_path_test(&self, context: &TestContext) -> TestCase {
        let mut test =
            TestCase::new(format!("test_{}_happy_path", context.function_name), TestType::Unit);

        test.description =
            format!("Test {} with valid inputs returns expected output", context.function_name);
    ...
    fn generate_error_case_test(&self, context: &TestContext) -> TestCase {
        let mut test =
            TestCase::new(format!("test_{}_error_case", context.function_name), TestType::Unit);

        test.description =
            format!("Test {} handles invalid inputs gracefully", context.function_name);
    ...
    fn generate_boundary_test(&self, context: &TestContext) -> TestCase {
        let mut test =
            TestCase::new(format!("test_{}_boundary", context.function_name), TestType::Unit);

        test.description = format!("Test {} with boundary values", context.function_name);
...
mod tests {
    ...
    fn test_happy_path_generation() {
        let strategy = UnitTestStrategy::new();
        let context = TestContext::new("add_numbers").with_parameter(ParameterInfo {
            name: "a".to_string(),
