...
impl CoverageAnalyzer {
    ...
    pub fn analyze_coverage(
        ...
        let all_requirements: HashSet<String> =
            spec.requirements.iter().map(|r| r.id.clone()).collect();

        // Find covered requirements
        let covered_requirements: HashSet<String> = test_suite
            .tests
            .iter()
        ...
        report.uncovered_requirements =
            all_requirements.difference(&covered_requirements).cloned().collect();
    ...
    fn extract_requirement_id(&self, description: &str) -> Option<String> {
        ...
        if let Some(start) = description.find("REQ-") {
            ...
            let end =
                id_part.find(|c: char| !c.is_alphanumeric() && c != '-').unwrap_or(id_part.len());
    ...
    pub fn generate_suggestions(&self, report: &CoverageReport) -> Vec<String> {
        ...
        if report.unit_tests == 0 {
            suggestions
                .push("No unit tests found. Add unit tests for individual functions.".to_string());
        }

        if report.integration_tests == 0 {
            suggestions.push(
                "No integration tests found. Add tests for component interactions.".to_string(),
            );
        }
...
mod tests {
    ...
    fn test_requirement_id_extraction() {
        ...
        assert_eq!(analyzer.extract_requirement_id("No requirement here"), None);
    }
