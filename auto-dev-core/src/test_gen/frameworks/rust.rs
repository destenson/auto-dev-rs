...
use crate::test_gen::{
    Assertion, AssertionType, Fixture, Property, TestCase, TestInput, TestSuite, TestType,
};
use anyhow::Result;
...
impl RustTestAdapter {
    pub fn new() -> Self {
        Self { use_tokio: false, use_proptest: true, use_quickcheck: false }
    }
    ...
    fn generate_test_case(&self, test: &TestCase) -> String {
        ...
        // Add async if needed
        let fn_prefix =
            if self.use_tokio && test.test_type != TestType::Property { "async " } else { "" };
    ...
    fn generate_assertion(&self, assertion: &Assertion) -> String {
        match assertion.assertion_type {
            ...
            AssertionType::Matches => {
                format!(
                    r#"assert!(Regex::new(r"{}").unwrap().is_match(&{}));"#,
