...
impl NativeModule {
    ...
    pub fn load(path: &Path) -> Result<Self> {
        ...
        let library = unsafe {
            Library::new(path)
                .context("Failed to load native library")?
        };
    ...
    fn get_module_metadata(library: &Library) -> Result<ModuleMetadata> {
        unsafe {
            ...
            let metadata: ModuleMetadata = serde_json::from_slice(&buffer)
                .context("Failed to deserialize module metadata")?;
    ...
    fn call_function(&self, func_name: &str, input: &[u8]) -> Result<Vec<u8>> {
        unsafe {
            let func: Symbol<ExecuteFn> = self.library
            ...
            pub extern "C" fn module_execute(
                ...
                unsafe {
                    if let Some(ref module) = MODULE_INSTANCE {
                        ...
                        std::ptr::copy_nonoverlapping(output_json.as_ptr(), output, output_json.len());
