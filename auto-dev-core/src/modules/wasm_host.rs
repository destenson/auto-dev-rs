...
impl WasmModule {
    ...
    fn setup_host_functions(&self, linker: &mut Linker<WasmState>) -> Result<()> {
        ...
        linker.func_wrap(
            "env",
            "allocate",
            |mut caller: wasmtime::Caller<'_, WasmState>, size: i32| -> Result<i32> {
                // Check if allocation is within limits
                let state = caller.data_mut();
                state.memory_used += size as u64;

                if state.memory_used > 100 * 1024 * 1024 {
                    // 100MB limit
        ...
        linker.func_wrap(
            "env",
            "deallocate",
            |mut caller: wasmtime::Caller<'_, WasmState>, _ptr: i32, size: i32| {
                let state = caller.data_mut();
    ...
    async fn call_function(&self, func_name: &str, input: &[u8]) -> Result<Vec<u8>> {
        ...
        func.call(
            &mut store,
            &[wasmtime::Val::I32(input_ptr), wasmtime::Val::I32(input.len() as i32)],
            &mut results,
        )
