...
impl HistorySearcher {
    ...
    pub fn get_commit_stats(&self, days: usize) -> Result<CommitStats> {
        ...
        stats.average_per_day =
            if days > 0 { stats.total_commits as f64 / days as f64 } else { 0.0 };
...
mod tests {
    ...
    fn test_search_builder() {
        ...
        let searcher =
            HistorySearcher { repo: unsafe { std::mem::zeroed() }, repo_path: PathBuf::new() };

        let _builder =
            SearchBuilder::new(&searcher).with_message("fix").with_author("alice").limit(10);
    }
}
