...
use super::orchestrator::{ChangeType, PendingChange, RiskLevel, TestResults};
use super::{Result, SelfDevConfig, SelfDevError};
use crate::safety::{CodeModification, ModificationType, SafetyConfig, SafetyGatekeeper};
...
impl ComponentCoordinator {
    ...
    pub async fn has_pending_work(&self) -> bool {
        ...
        let has_pending = if prp_dir.exists() { true } else { false };
    ...
    pub async fn deploy_approved_changes(&self) -> Result<()> {
        ...
        if *today_count >= self.config.max_changes_per_day {
            return Err(SelfDevError::Coordination(format!(
                ...
                self.config.max_changes_per_day
            )));
        }
