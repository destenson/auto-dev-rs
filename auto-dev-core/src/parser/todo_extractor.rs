...
impl TodoExtractor {
    ...
    pub fn extract_from_content(&self, content: &str, source_path: &Path) -> Result<Vec<Requirement>> {
        ...
        for (line_num, line) in content.lines().enumerate() {
            if let Some(todo) = self.extract_todo_from_line(line, line_num + 1) {
                ...
                requirement.source_location = SourceLocation::new(
    ...
    fn infer_category(&self, description: &str) -> RequirementType {
        ...
        if desc_lower.contains("implement") || desc_lower.contains("add") || 
...
mod tests {
    ...
    fn test_extract_todo_from_line() {
        ...
        let test_cases = vec![
            ("// TODO: Implement feature X", Some(("TODO".to_string(), "Implement feature X".to_string()))),
            ("// FIXME: Critical bug in Y", Some(("FIXME".to_string(), "Critical bug in Y".to_string()))),
            ("# TODO: Add documentation", Some(("TODO".to_string(), "Add documentation".to_string()))),
            ("/* HACK: Temporary workaround */", Some(("HACK".to_string(), "Temporary workaround".to_string()))),
    ...
    fn test_category_inference() {
        ...
        assert_eq!(
            extractor.infer_category("Implement new feature"),
            RequirementType::Functional
        );
        assert_eq!(
            extractor.infer_category("Fix memory leak"),
            RequirementType::Reliability
        );
        assert_eq!(
            extractor.infer_category("Add API endpoint"),
            RequirementType::Api
        );
        assert_eq!(
            extractor.infer_category("Improve security checks"),
            RequirementType::Security
        );
    }
