...
impl BaselineCreator {
    ...
    fn capture_configuration(&self) -> Result<ConfigurationSnapshot> {
        ...
        let cargo_toml = fs::read_to_string("Cargo.toml").map_err(|e| {
            BootstrapError::BaselineFailed(format!("Failed to read Cargo.toml: {}", e))
        })?;
    ...
    async fn capture_performance_metrics(&self) -> Result<PerformanceMetrics> {
        ...
        let output = Command::new("cargo").args(&["build", "--release"]).output().map_err(|e| {
            BootstrapError::BaselineFailed(format!("Failed to build for metrics: {}", e))
        })?;
    ...
    fn calculate_binary_hash(&self) -> Result<String> {
        ...
        if !binary_path.exists() {
            ...
            if debug_path.exists() {
                let content = fs::read(debug_path).map_err(|e| {
                    BootstrapError::BaselineFailed(format!("Failed to read binary: {}", e))
                })?;
        ...
        let content = fs::read(binary_path)
            .map_err(|e| BootstrapError::BaselineFailed(format!("Failed to read binary: {}", e)))?;
    ...
    fn create_rollback_point(&self, snapshot_id: &str) -> Result<RollbackPoint> {
        ...
        fs::create_dir_all(&backup_dir).map_err(|e| {
            BootstrapError::BaselineFailed(format!("Failed to create backup directory: {}", e))
        })?;

        // Backup configuration
        let config_backup = backup_dir.join("config_backup.toml");
        if Path::new("Cargo.toml").exists() {
            fs::copy("Cargo.toml", &config_backup).map_err(|e| {
                BootstrapError::BaselineFailed(format!("Failed to backup Cargo.toml: {}", e))
            })?;
        }
    ...
    fn save_baseline(&self, baseline: &Baseline) -> Result<()> {
        ...
        let json = serde_json::to_string_pretty(baseline).map_err(|e| {
            BootstrapError::BaselineFailed(format!("Failed to serialize baseline: {}", e))
        })?;

        fs::write(&baseline_path, json).map_err(|e| {
            BootstrapError::BaselineFailed(format!("Failed to save baseline: {}", e))
        })?;

        // Also update the "latest" symlink/reference
        let latest_path = self.config.snapshot_dir.join("latest.json");
        fs::write(&latest_path, serde_json::to_string_pretty(baseline).unwrap()).map_err(|e| {
            BootstrapError::BaselineFailed(format!("Failed to update latest baseline: {}", e))
        })?;
    ...
    fn log_audit_entry(&self, entry: BootstrapAuditEntry) -> Result<()> {
        let json_line = serde_json::to_string(&entry).map_err(|e| {
            BootstrapError::BaselineFailed(format!("Failed to serialize audit entry: {}", e))
        })?;

        // Append to audit log (preserves all context)
        use std::io::Write;
        let mut file =
            fs::OpenOptions::new().create(true).append(true).open(&self.audit_log_path).map_err(
                |e| BootstrapError::BaselineFailed(format!("Failed to open audit log: {}", e)),
            )?;

        writeln!(file, "{}", json_line).map_err(|e| {
            BootstrapError::BaselineFailed(format!("Failed to write audit entry: {}", e))
        })?;
