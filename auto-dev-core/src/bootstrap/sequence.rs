...
impl BootstrapSequence {
    ...
    async fn run_baseline_stage(&mut self) -> Result<()> {
        ...
        let attempt_number = previous_context
            .as_ref()
            .map(|c| c.bootstrap_attempt + 1)
            .unwrap_or(1);
        
        let context = SnapshotContext {
            reason: "Bootstrap initialization".to_string(),
            previous_snapshot_id: previous_context.as_ref().map(|c| c.previous_snapshot_id.clone()).flatten(),
    ...
    async fn run_activation_stage(&mut self) -> Result<()> {
        ...
        if self.config.monitoring.start_immediately {
            ...
            info!("Monitoring will start after {} seconds delay", 
        ...
        let orchestrator = crate::self_dev::initialize(self_dev_config).await
            .map_err(|e| BootstrapError::ActivationFailed(
                format!("Failed to initialize self-dev: {}", e)
            ))?;
    ...
    fn is_already_running(&self) -> Result<bool> {
        ...
        // Create lock file
        fs::write(lock_file, chrono::Utc::now().to_rfc3339())
