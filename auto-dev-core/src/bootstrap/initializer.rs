...
impl SystemInitializer {
    pub fn new(
        ...
        Self { safety_config, modules_config, monitoring_config }
    }
        ...
        for dir in dirs {
            if !Path::new(dir).exists() {
                fs::create_dir_all(dir).map_err(|e| {
                    BootstrapError::InitializationFailed(format!(
    ...
    async fn initialize_module_system(&self) -> Result<()> {
        ...
        if !registry_path.exists() {
            ...
            fs::write(registry_path, serde_json::to_string_pretty(&initial_registry).unwrap())
                .map_err(|e| {
                    BootstrapError::InitializationFailed(format!(
                        "Failed to create module registry: {}",
                        e
                    ))
                })?;
...
max_open_files = 100
"#;
                fs::write(sandbox_config_path, config_content).map_err(|e| {
                    BootstrapError::InitializationFailed(format!(
...
"#,
            ...
            fs::write(monitor_config_path, config_content).map_err(|e| {
                BootstrapError::InitializationFailed(format!(
                    "Failed to create monitor config: {}",
                    e
                ))
            })?;
...
"#,
            ...
            fs::write(safety_config_path, config_content).map_err(|e| {
                BootstrapError::InitializationFailed(format!(
                    "Failed to create safety config: {}",
                    e
                ))
            })?;
