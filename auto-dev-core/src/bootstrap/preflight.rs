...
impl PreflightChecker {
    ...
    fn check_rust_toolchain(&self) -> Result<()> {
        ...
        let output = Command::new("rustc").arg("--version").output().map_err(|e| {
            BootstrapError::PreflightFailed(format!("Failed to check Rust toolchain: {}", e))
        })?;
        ...
        // Check cargo
        let cargo_output = Command::new("cargo").arg("--version").output().map_err(|e| {
            BootstrapError::PreflightFailed(format!("Failed to check cargo: {}", e))
        })?;

        if !cargo_output.status.success() {
            return Err(BootstrapError::PreflightFailed("Cargo not available".to_string()));
        }
    ...
    fn check_disk_space(&self) -> Result<()> {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            let output = Command::new("wmic")
                ...
                .output()
                .map_err(|e| {
                    BootstrapError::PreflightFailed(format!("Failed to check disk space: {}", e))
                })?;
        ...
        #[cfg(not(target_os = "windows"))]
        {
            ...
            let output = Command::new("df").args(&["-k", "."]).output().map_err(|e| {
                BootstrapError::PreflightFailed(format!("Failed to check disk space: {}", e))
            })?;
    ...
    fn check_git_state(&self) -> Result<()> {
        ...
        let status_output =
            Command::new("git").args(&["status", "--porcelain"]).output().map_err(|e| {
                BootstrapError::PreflightFailed(format!("Failed to check git status: {}", e))
            })?;

        if !status_output.status.success() {
            return Err(BootstrapError::PreflightFailed("Not in a git repository".to_string()));
        }
    ...
    fn run_tests(&self) -> Result<()> {
        ...
        let output = Command::new("cargo")
            .args(&["test", "--quiet"])
            .output()
            .map_err(|e| BootstrapError::PreflightFailed(format!("Failed to run tests: {}", e)))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(BootstrapError::PreflightFailed(format!("Test suite failed:\n{}", stderr)));
        }
    ...
    fn check_configuration(&self) -> Result<()> {
        ...
        if !auto_dev_dir.exists() {
            std::fs::create_dir_all(auto_dev_dir).map_err(|e| {
                BootstrapError::PreflightFailed(format!("Cannot create .auto-dev directory: {}", e))
            })?;
    ...
    pub fn describe_checks(&self) -> Vec<String> {
        let mut checks =
            vec!["Rust toolchain availability".to_string(), "Configuration validity".to_string()];
