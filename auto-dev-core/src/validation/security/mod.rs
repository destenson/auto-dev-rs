...
use crate::validation::{ValidationResult, ValidationError, Severity, ErrorCategory};
use anyhow::Result;
        ...
        match output {
            ...
            Ok(output) => {
                ...
                Ok(ValidationResult::failed(ValidationError {
                    ...
                    fix_suggestion: Some("Run 'cargo audit fix' to attempt automatic fixes".to_string()),
                }))
            }
            Err(_) => {
