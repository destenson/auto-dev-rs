...
use crate::validation::{
    ValidationResult, ValidationError, ValidationWarning, QualityMetrics, 
    Severity, ErrorCategory, SourceLocation, Improvement, Priority
};
use anyhow::Result;
    ...
    async fn run_clippy(&self) -> Result<ValidationResult> {
        ...
        let output = Command::new("cargo")
            .args(&[
                ...
                "-A", "clippy::must_use_candidate",       // Too noisy
            ])
    ...
    async fn check_outdated_deps(&self) -> Result<ValidationResult> {
        ...
        if check.is_err() {
            result.suggestions.push(Improvement {
                ...
                description: "Install cargo-outdated with: cargo install cargo-outdated".to_string(),
                priority: Priority::Low,
            });
            return Ok(result);
        }
                ...
                if let Some(deps) = json["dependencies"].as_array() {
                    ...
                    if outdated_count > 0 {
                        ...
                        if outdated_count > 10 {
                            result.suggestions.push(Improvement {
                                category: "dependencies".to_string(),
                                description: "Consider updating dependencies with 'cargo update'".to_string(),
                    ...
                    if total_lines > 0 {
                        ...
                        if comment_ratio < 10.0 {
                            ...
                            result.suggestions.push(Improvement {
                                ...
                                description: "Consider adding more documentation comments".to_string(),
                                priority: Priority::Medium,
                            });
                        }
                    }
