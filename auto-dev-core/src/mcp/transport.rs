...
impl StdioTransport {
    ...
    pub async fn spawn(command: &str, args: Vec<String>) -> Result<Self> {
        ...
        Ok(Self { process: Arc::new(Mutex::new(child)), request_tx, pending_requests })
    }
}
...
impl McpTransport for StdioTransport {
    async fn send_request(&self, method: &str, params: Value) -> Result<Value> {
        ...
        let request = Request { id: Some(id), message, response_tx };

        self.request_tx.send(request).await.context("Failed to send request")?;

        // Wait for response
        let response =
            response_rx.recv().await.ok_or_else(|| anyhow::anyhow!("No response received"))?;
    ...
    async fn send_notification(&self, method: &str, params: Value) -> Result<()> {
        ...
        let request = Request { id: None, message, response_tx };

        self.request_tx.send(request).await.context("Failed to send notification")?;
