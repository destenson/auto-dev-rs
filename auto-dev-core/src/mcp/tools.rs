...
impl WebSearchTool {
    ...
    /// Search the web
    pub async fn search(&self, query: &str, max_results: Option<usize>) -> Result<Vec<SearchResult>> {
...
impl DatabaseTool {
    ...
    /// Execute a SQL query
    pub async fn query(&self, sql: &str, params: Option<Vec<serde_json::Value>>) -> Result<QueryResult> {
        let client = self.client.read().await;
    ...
    pub async fn get_schema(&self, table: Option<&str>) -> Result<serde_json::Value> {
        ...
        let arguments = if let Some(table) = table {
            json!({ "table": table })
        } else {
            json!({})
        };
