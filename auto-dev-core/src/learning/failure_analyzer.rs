...
use crate::learning::pattern_extractor::{Pattern, PatternType, PatternContext};
...
impl FailureAnalyzer {
    ...
    pub fn extract_anti_pattern(&self, event: &LearningEvent, cause: &FailureCause) -> Option<AntiPattern> {
    ...
    pub fn get_anti_patterns_for_context(&self, context: &PatternContext) -> Vec<&AntiPattern> {
        self.anti_patterns
            .iter()
            .filter(|ap| ap.context.matches(context))
            .collect()
    }
...
fn extract_stack_trace(event: &LearningEvent) -> Option<String> {
    event.outcome.details.get("stack_trace")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
}
...
fn extract_vulnerability_type(details: &serde_json::Value) -> String {
    details.get("vulnerability_type")
        .and_then(|v| v.as_str())
        .unwrap_or("unknown")
        .to_string()
}
...
fn extract_test_output(event: &LearningEvent) -> String {
    event.outcome.details.get("output")
        .and_then(|v| v.as_str())
...
fn generate_guard_conditions(cause: &FailureCause, context: &crate::learning::learner::EventContext) -> Vec<GuardCondition> {
