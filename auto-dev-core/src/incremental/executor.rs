...
use super::{
    Increment, Result, IncrementalError, IncrementStatus, AttemptResult,
...
impl IncrementExecutor {
        ...
        for check in &increment.validation.security_checks {
            ...
            if !result.passed {
                return Err(IncrementalError::ValidationError(format!(
                    "Security check '{}' failed: {}", check.name, result.message
                )));
            }
        }
    ...
    fn determine_default_path(&self, increment: &Increment) -> PathBuf {
        let base_name = increment.specification.id
            .chars()
