...
use super::{IncrementalError, Result};
...
impl RollbackManager {
    ...
    pub fn new(project_root: PathBuf) -> Result<Self> {
        ...
        if !Self::is_git_available(&project_root) {
            return Err(IncrementalError::RollbackError(
                "Git is required for rollback functionality. Please initialize a git repository."
                    .to_string(),
            ));
        }
    ...
    async fn create_git_checkpoint(&self, checkpoint: &CheckpointId) -> Result<()> {
        ...
        let output = Command::new("git")
            ...
            .await
            .map_err(|e| {
                IncrementalError::RollbackError(format!("Failed to stage changes: {}", e))
            })?;
        ...
        let output = Command::new("git")
            ...
            .await
            .map_err(|e| {
                IncrementalError::RollbackError(format!("Failed to create stash: {}", e))
            })?;
    ...
    async fn rollback_git_checkpoint(&self, checkpoint: &CheckpointId) -> Result<()> {
        ...
        let output = Command::new("git")
            ...
            .await
            .map_err(|e| {
                IncrementalError::RollbackError(format!("Failed to list stashes: {}", e))
            })?;

        let stash_list = String::from_utf8_lossy(&output.stdout);
        let stash_index = stash_list.lines().position(|line| line.contains(&stash_name));
        ...
        if let Some(index) = stash_index {
            ...
            let output = Command::new("git")
                ...
                .await
                .map_err(|e| {
                    IncrementalError::RollbackError(format!("Failed to apply stash: {}", e))
                })?;
...
impl CheckpointId {
    ...
    pub fn new() -> Self {
        Self { id: Uuid::new_v4(), created_at: Utc::now(), description: None }
    }
