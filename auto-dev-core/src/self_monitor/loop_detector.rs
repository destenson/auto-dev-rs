...
impl LoopDetector {
    pub fn new(config: LoopDetectorConfig) -> Self {
        Self { modification_windows: HashMap::new(), config, cooldown_files: HashMap::new() }
    }
    ...
    pub fn check_modification(&mut self, path: &Path) -> Result<LoopDetectionResult> {
        ...
        if let Some(cooldown_end) = self.cooldown_files.get(path) {
            if now < *cooldown_end {
                let remaining = cooldown_end.duration_since(now).unwrap_or(Duration::from_secs(0));
        ...
        let window = self.modification_windows.entry(path.to_path_buf()).or_insert_with(|| {
            ModificationWindow::new(Duration::from_secs(self.config.window_duration_secs))
        });
        ...
        if recent_count >= self.config.max_modifications_per_window {
            warn!("Loop detected for file: {:?}, {} modifications in window", path, recent_count);
    ...
    pub fn cleanup_old_data(&mut self) {
        ...
        self.modification_windows.retain(|_, window| window.has_recent_activity());
    }
}
...
impl ModificationWindow {
    fn new(window_duration: Duration) -> Self {
        Self { timestamps: VecDeque::new(), window_duration, content_hashes: VecDeque::new() }
    }
    ...
    fn has_burst_pattern(&self) -> bool {
        ...
        for i in 1..self.timestamps.len() {
            if let (Some(prev), Some(curr)) = (self.timestamps.get(i - 1), self.timestamps.get(i)) {
...
pub enum LoopDetectionResult {
    ...
    LoopDetected { modification_count: usize, window_duration: Duration },
