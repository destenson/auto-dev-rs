...
mod tests {
    use super::*;
    use crate::context::{
        analyzer::{CodePattern, CodingConventions, NamingStyle, PatternType},
        manager::{ArchitectureDecision, ContextManager, ContextUpdate},
        query::{ComplexQuery, ContextQuery},
        storage::{ContextStorage, ProjectContext},
    };
    use chrono::Utc;
...
"#,
    ...
    async fn test_project_structure_analysis() {
        ...
        assert!(matches!(
            ...
            NamingStyle::SnakeCase | NamingStyle::Unknown
        ));
    }
