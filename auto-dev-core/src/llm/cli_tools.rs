...
{}",
    ...
    async fn assess_complexity(&self, task: &str) -> Result<TaskComplexity> {
        let prompt =
            format!("Assess if this task is simple, moderate, complex, or very complex: {}", task);
        let response = self.run_claude(&prompt).await?;
        ...
        Ok(TaskComplexity { tier, reasoning: response, estimated_tokens: 0, confidence: 0.7 })
    }
}
...
impl FabricProvider {
    ...
    async fn run_fabric(&self, input: &str, pattern: &str) -> Result<String> {
        ...
        let output = cmd.output().await.context("Failed to run fabric CLI")?;
    ...
    fn find_best_model_for_tier(&self, tier: ModelTier) -> Option<String> {
        ...
        let matching: Vec<String> =
            categorized.iter().filter(|(_, t)| *t == tier).map(|(m, _)| m.clone()).collect();
...
{}",
    ...
    async fn assess_complexity(&self, task: &str) -> Result<TaskComplexity> {
        ...
        Ok(TaskComplexity { tier, reasoning: response, estimated_tokens: 0, confidence: 0.6 })
    }
}
...
impl GenericCLIProvider {
    pub async fn new(command: String, args_template: Vec<String>, name: String) -> Self {
        ...
        Self { command, args_template, available, name }
    }
    ...
    async fn run_command(&self, input: &str) -> Result<String> {
        ...
        let output = cmd.output().await.context(format!("Failed to run {} CLI", self.name))?;
...
impl LLMProvider for GenericCLIProvider {
    ...
    async fn generate_code(
        ...
        let input = format!("Generate {} code: {}", context.language, spec.content);
    ...
    async fn review_code(&self, code: &str, requirements: &[Requirement]) -> Result<ReviewResult> {
        let req_text =
            requirements.iter().map(|r| r.description.clone()).collect::<Vec<_>>().join(", ");
