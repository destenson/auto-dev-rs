...
pub enum DetectionError {
    #[error(
        "Claude Code CLI not found. Install from: https://docs.anthropic.com/en/docs/claude-code/quickstart"
    )]
    NotFound,
...
impl ClaudeDetector {
    ...
    fn detect_npm_global() -> Result<Option<ClaudeLocation>> {
        let npm_paths = if cfg!(windows) {
            vec![
                // Windows NPM paths
                env::var("APPDATA").ok().map(|p| Path::new(&p).join("npm").join("claude.cmd")),
                env::var("APPDATA").ok().map(|p| Path::new(&p).join("npm").join("claude.exe")),
            ]
        } else {
            vec![
                // Unix NPM paths
    ...
    fn validate_binary(path: &Path) -> Result<ClaudeLocation> {
        ...
        Ok(ClaudeLocation { path: path.to_path_buf(), version, version_output })
    }
    ...
    fn parse_version(output: &str) -> Result<String> {
        ...
        Err(DetectionError::VersionParseError { output: output.to_string() }.into())
    }
