...
impl TokenManager {
    ...
    pub fn new() -> Self {
        ...
        Self { encoders: HashMap::new(), model_limits }
    }
...
impl ConversationManager {
    ...
    pub fn set_system(&mut self, prompt: String) {
        ...
        self.messages
            .insert(0, Message { role: "system".to_string(), content: prompt, name: None });
    }

    /// Add a user message
    pub fn add_user(&mut self, content: String) {
        self.messages.push(Message { role: "user".to_string(), content, name: None });
    }

    /// Add an assistant message
    pub fn add_assistant(&mut self, content: String) {
        self.messages.push(Message { role: "assistant".to_string(), content, name: None });
    }
...
mod tests {
    ...
    fn test_message_tokens() {
        ...
        let messages = vec![
            ...
            Message { role: "user".to_string(), content: "Hello!".to_string(), name: None },
        ];
    ...
    fn test_trim_messages() {
        ...
        if trimmed.len() > 1 && messages.len() > 1 {
            ...
            let matching_original =
                messages.iter().rev().find(|m| m.content == last_trimmed.content);
