...
use super::{
    Result, SynthesisError, SynthesisResult, ArchitectureDecision,
...
impl SynthesisEngine {
    ...
    pub async fn synthesize_all(&mut self, specs: Vec<Specification>) -> Result<Vec<SynthesisResult>> {
    ...
    pub async fn status(&self) -> Result<SynthesisStatus> {
        ...
        Ok(SynthesisStatus {
            ...
            completed_specifications: state.specifications.values()
    ...
    pub async fn rollback(&mut self, checkpoint_id: &str) -> Result<()> {
        if !self.config.enable_rollback {
            return Err(SynthesisError::StateError(
    ...
    async fn build_result(&self, context: PipelineContext) -> Result<SynthesisResult> {
        Ok(SynthesisResult {
            ...
            tasks_completed: context.completed_tasks.iter()
