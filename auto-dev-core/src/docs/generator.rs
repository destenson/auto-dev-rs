...
use super::{ApiDoc, ApiItemKind, DocConfig, DocEntry, DocFormat, DocSection, Example};
...
impl DocGenerator {
    ...
    pub fn generate_api_docs(
        ...
        // Group by kind
        let mut by_kind: std::collections::HashMap<ApiItemKind, Vec<&ApiDoc>> =
    ...
    pub fn update_readme(&self, readme_path: &Path) -> Result<()> {
        let content =
            if readme_path.exists() { fs::read_to_string(readme_path)? } else { String::new() };
    ...
    fn generate_index(&self, entries: &[DocEntry], output_dir: &Path) -> Result<PathBuf> {
        ...
        for entry in entries {
            content.push_str(&format!(
                "- [{}]({}.md) - {}\n",
                entry.metadata.name, entry.metadata.name, entry.metadata.description
            ));
        }
