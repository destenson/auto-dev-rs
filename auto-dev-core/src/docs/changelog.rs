...
impl ChangelogBuilder {
    ...
    pub fn load(&mut self) -> Result<()> {
        ...
        let content =
            fs::read_to_string(&self.config.file_path).context("Failed to read changelog")?;
    ...
    pub fn update(&self) -> Result<()> {
        ...
        fs::write(&self.config.file_path, content).context("Failed to write changelog")?;
        Ok(())
    }
    ...
    fn get_or_create_unreleased(&mut self) -> &mut ChangelogEntry {
        ...
        self.entries.iter_mut().find(|e| e.version == "Unreleased").unwrap()
    }
    ...
    fn parse_changelog(&self, content: &str) -> Result<Vec<ChangelogEntry>> {
        ...
        for line in content.lines() {
            ...
            if line.starts_with("## ") {
                ...
                current_entry = Some(ChangelogEntry { version, date, changes: HashMap::new() });
                ...
                && current_entry.is_some()
            {
                ...
                let change = Change { description, references: vec![], author: None };
    ...
    fn format_entry(&self, entry: &ChangelogEntry) -> String {
        ...
        if let Some(date) = entry.date {
            content.push_str(&format!("## [{}] - {}\n\n", entry.version, date.format("%Y-%m-%d")));
        } else {
            content.push_str(&format!("## [{}]\n\n", entry.version));
        }
...
mod tests {
    ...
    fn test_release() {
        ...
        builder.add_change(ChangeCategory::Added, "Feature Y".to_string(), vec![]).unwrap();
