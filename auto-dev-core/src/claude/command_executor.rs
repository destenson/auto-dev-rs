...
impl CommandExecutor {
    ...
    fn detect_command_type(&self) -> CommandType {
        ...
        if instructions.contains("echo")
            || instructions.contains("ls")
            || instructions.contains("cd")
    ...
    async fn execute_shell(&self, args: HashMap<String, String>) -> Result<CommandOutput> {
        ...
        let shell = if cfg!(windows) { ("cmd", vec!["/C"]) } else { ("sh", vec!["-c"]) };
        ...
        let output = cmd.output().await.context("Failed to execute shell command")?;
    ...
    fn detect_script_type(&self, content: &str) -> ScriptType {
        ...
        {
            ScriptType::Python
        } else if first_line.starts_with("#!/usr/bin/env node")
            || first_line.starts_with("#!/usr/bin/node")
        {
            ScriptType::JavaScript
        } else if first_line.starts_with("#!/usr/bin/env bash")
...
impl CommandOutput {
    ...
    pub fn success(stdout: String) -> Self {
        Self { success: true, stdout, stderr: String::new(), exit_code: Some(0) }
    }

    /// Create a failure output
    pub fn failure(stderr: String) -> Self {
        Self { success: false, stdout: String::new(), stderr, exit_code: Some(1) }
    }
...
mod tests {
    ...
    fn test_script_type_detection() {
        ...
        assert_eq!(executor.detect_script_type("#!/bin/bash\necho hello"), ScriptType::Bash);
    }
}
