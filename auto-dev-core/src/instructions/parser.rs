...
use crate::instructions::formats::{
    Format, detect_format, parse_json, parse_markdown, parse_text, parse_yaml,
};
...
impl InstructionParser {
    ...
    fn parse_content(raw_content: String, format: Format) -> Result<ParsedInstruction> {
        ...
        Ok(ParsedInstruction { raw_content, format, sections, metadata, instruction_text })
    }
...
impl ParsedInstruction {
    ...
    pub fn find_sections(&self, title: &str) -> Vec<&InstructionSection> {
        ...
        self.sections.iter().filter(|s| s.title.to_lowercase().contains(&title_lower)).collect()
    }

    /// Get the main instruction content
    pub fn get_main_content(&self) -> &str {
        if !self.instruction_text.is_empty() { &self.instruction_text } else { &self.raw_content }
    }
