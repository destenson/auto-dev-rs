...
impl TimeSeriesStore {
    pub fn new(storage_path: impl AsRef<Path>) -> Result<Self> {
        ...
        std::fs::create_dir_all(&path).map_err(|e| {
            MetricsError::Storage(format!("Failed to create storage directory: {}", e))
        })?;
    ...
    pub fn get_latest(&self, series_name: &str) -> Option<MetricPoint> {
        self.series.get(series_name).and_then(|series| series.last().cloned())
    }
    ...
    fn load_from_disk(&mut self) -> Result<()> {
        let entries = std::fs::read_dir(&self.storage_path).map_err(|e| {
            MetricsError::Storage(format!("Failed to read storage directory: {}", e))
        })?;
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                if let Some(name) = path.file_stem().and_then(|s| s.to_str()) {
                    ...
                    match std::fs::read_to_string(&path) {
                        Ok(content) => match serde_json::from_str::<Vec<MetricPoint>>(&content) {
                            Ok(series) => {
