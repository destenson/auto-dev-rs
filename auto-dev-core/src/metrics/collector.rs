...
use super::{
    DevelopmentMetrics, QualityMetrics, PerformanceMetrics, CapabilityMetrics,
    MetricsSnapshot, MetricsError, Result, MetricType
};
...
impl MetricsCollector {
    pub async fn new() -> Result<Self> {
        ...
        std::fs::create_dir_all(&storage_path)
            .map_err(|e| MetricsError::Storage(format!("Failed to create metrics directory: {}", e)))?;
    ...
    pub async fn get_current_snapshot(&self) -> Result<MetricsSnapshot> {
        ...
        snapshots.last().cloned().ok_or_else(|| {
            MetricsError::Collection("No metrics snapshot available".to_string())
        })
    }
    ...
    async fn calculate_development_metrics(&self, events: &[MetricEvent]) -> DevelopmentMetrics {
        ...
        let success_rate = if total_events > 0.0 {
        ...
        let modifications_per_day = if let (Some(first), Some(last)) = (events.first(), events.last()) {
        ...
        let rollback_events = events.iter()
            .filter(|e| e.event_type.contains("rollback"))
            .count() as f64;
        let rollback_frequency = if total_events > 0.0 {
            rollback_events / total_events
        } else {
            0.0
        };
    ...
    async fn calculate_capability_metrics(&self, events: &[MetricEvent]) -> CapabilityMetrics {
        let features_added = events.iter()
            .filter(|e| e.event_type.contains("feature_add"))
            .count() as u32;
            
        let apis_created = events.iter()
            .filter(|e| e.event_type.contains("api_create"))
            .count() as u32;
            
        let modules_loaded = events.iter()
            .filter(|e| e.event_type.contains("module_load"))
            .count() as u32;
            
        let patterns_learned = events.iter()
            .filter(|e| e.event_type.contains("pattern_learn"))
            .count() as u32;
            
        let llm_calls_saved = events.iter()
            .filter(|e| e.event_type.contains("llm_cache_hit"))
            .count() as u32;
