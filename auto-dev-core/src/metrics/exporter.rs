...
use super::{MetricsSnapshot, MetricsError, Result};
...
impl MetricsExporter {
    ...
    fn export_html(snapshots: &[MetricsSnapshot]) -> Result<String> {
        ...
        if let Some(latest) = snapshots.last() {
            ...
            html.push_str(&format!("<span class='metric-value'>{:.1}%</span></div>\n", latest.development.success_rate * 100.0));
            html.push_str("<div class='metric'><span class='metric-label'>Modifications/Day:</span> ");
            html.push_str(&format!("<span class='metric-value'>{:.2}</span></div>\n", latest.development.modifications_per_day));
