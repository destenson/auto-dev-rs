...
use super::{MetricsError, Result, MetricPoint};
...
impl TrendAnalyzer {
    ...
    fn calculate_trend(&self, points: &[MetricPoint]) -> Result<TrendAnalysis> {
        ...
        let x_values: Vec<f64> = points
            .iter()
            .map(|p| (p.timestamp - first_timestamp).num_seconds() as f64)
            .collect();
    ...
    pub fn detect_anomalies(
        ...
        points
            .into_iter()
            .filter(|p| (p.value - mean).abs() > threshold_std * std_dev)
            .collect()
    }
    ...
    pub fn compare_periods(
        ...
        let period1_avg = period1_points.iter().map(|p| p.value).sum::<f64>() / period1_points.len() as f64;
        let period2_avg = period2_points.iter().map(|p| p.value).sum::<f64>() / period2_points.len() as f64;
        
        let change = period2_avg - period1_avg;
        let change_percent = if period1_avg != 0.0 {
            (change / period1_avg) * 100.0
        } else {
            0.0
        };
    ...
    pub fn find_correlation(
        ...
        for p1 in &points1 {
            ...
            if let Some(p2) = points2.iter().min_by_key(|p| {
                (p.timestamp - p1.timestamp).num_seconds().abs()
            }) {
        ...
        if values1.len() < 2 {
            return Err(MetricsError::Analysis("No aligned data points for correlation".to_string()));
        }
