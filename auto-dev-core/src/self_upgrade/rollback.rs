...
impl RollbackManager {
    pub fn new(binary_path: PathBuf, keep_versions: usize) -> Self {
        Self { binary_path, backup_dir: PathBuf::from(".auto-dev/backups"), keep_versions }
    }
    ...
    pub async fn create_backup(&self) -> Result<PathBuf> {
        ...
        if self.binary_path.exists() {
            std::fs::copy(&self.binary_path, &backup_path).context("Failed to create backup")?;
    ...
    fn get_latest_backup(&self) -> Result<Option<PathBuf>> {
        ...
        let mut backups: Vec<_> = std::fs::read_dir(&self.backup_dir)?
            ...
            .filter(|entry| entry.file_name().to_string_lossy().starts_with("auto-dev_"))
            .collect();
