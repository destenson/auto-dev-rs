...
impl SelfTargetConfig {
    ...
    pub fn from_cargo_metadata() -> Result<Self> {
        let metadata = MetadataCommand::new().exec().context("Failed to execute cargo metadata")?;

        let root_package = metadata.root_package().context("No root package found")?;

        let workspace_root = metadata.workspace_root.as_std_path().to_path_buf();
        let project_path = root_package
            .manifest_path
            .parent()
        ...
        let monitor = MonitorConfig {
            ...
            exclude: vec!["target/".to_string(), ".git/".to_string(), "*.bak".to_string()],
            debounce_ms: 1000,
        };
        ...
        Ok(Self { project: project_info, monitor, analyzer, synthesis, safety })
    }
    ...
    pub fn from_file(path: &Path) -> Result<Self> {
        let content = fs::read_to_string(path).context("Failed to read self-target config file")?;

        let config: Self =
            toml::from_str(&content).context("Failed to parse self-target config")?;
    ...
    pub fn save(&self, path: &Path) -> Result<()> {
        ...
        let content =
            toml::to_string_pretty(self).context("Failed to serialize self-target config")?;

        fs::write(path, content).context("Failed to write self-target config file")?;
