    ...
    fn build_prompt(&self, event: &Event) -> String {
        match event.event_type {
            EventType::SpecificationChanged => {
                "Analyze this specification change and determine what implementation is needed."
                    .to_string()
            }
            EventType::TestAdded => {
            ...
            _ => "Analyze this event and determine the appropriate action.".to_string(),
        }
    }
...
impl DecisionRule for TestAddedRule {
    ...
    async fn evaluate(&self, event: &Event) -> Result<Option<Decision>> {
        ...
        Ok(Some(Decision::UpdateTests(vec![TestUpdate {
...
impl PatternMatcher {
    fn new() -> Self {
        Self { patterns: Arc::new(RwLock::new(HashMap::new())) }
    }
...
mod tests {
    ...
    async fn test_pattern_matching() {
        ...
        let event = Event::new(EventType::TestAdded, PathBuf::from("test_file.rs"));
