...
impl PerformanceBenchmark {
    ...
    ) -> Result<Vec<BenchmarkResult>, SelfTestError> {
    ...
    async fn benchmark_compilation(
        &self,
        sandbox: &mut TestSandbox,
    ) -> Result<BenchmarkResult, SelfTestError> {
    ...
    /// Benchmark test execution time
    async fn benchmark_test_execution(
        &self,
        sandbox: &mut TestSandbox,
    ) -> Result<BenchmarkResult, SelfTestError> {
    ...
    async fn benchmark_module_loading(
        &self,
        sandbox: &mut TestSandbox,
    ) -> Result<BenchmarkResult, SelfTestError> {
        ...
        let start = Instant::now();
        let result = sandbox
            .run_command("cargo", &["run", "--release", "--", "benchmark", "module-loading"])
            .await?;
        let duration_ms = start.elapsed().as_millis() as u64;
    ...
    async fn benchmark_monitoring_response(
        &self,
        sandbox: &mut TestSandbox,
    ) -> Result<BenchmarkResult, SelfTestError> {
    ...
    /// Benchmark LLM call overhead
    async fn benchmark_llm_overhead(
        &self,
        sandbox: &mut TestSandbox,
    ) -> Result<BenchmarkResult, SelfTestError> {
        ...
        let start = Instant::now();
        let result = sandbox
            .run_command("cargo", &["test", "--release", "--", "llm::benchmarks::call_overhead"])
            .await?;
        let duration_ms = start.elapsed().as_millis() as u64;
...
impl BenchmarkResult {
    ...
    pub fn summary(&self) -> String {
        format!("{}: {:.2} {} (took {}ms)", self.name, self.value, self.unit, self.duration_ms)
    }
}
