...
impl TestSandbox {
    ...
    pub async fn setup_project(&mut self, source_path: &Path) -> Result<(), SelfTestError> {
        ...
        if target_dir.exists() {
            fs::remove_dir_all(&target_dir).await
                .map_err(|e| SelfTestError::Sandbox(format!("Failed to remove target dir: {}", e)))?;
        }
    ...
    pub async fn apply_modifications(&mut self, modifications: Vec<FileModification>) -> Result<(), SelfTestError> {
        for modification in modifications {
            ...
            match modification.operation {
                ModificationOp::Create | ModificationOp::Update => {
                    ...
                    fs::write(&file_path, &modification.content)
                        .await
                ...
                ModificationOp::Delete => {
                    if file_path.exists() {
                        fs::remove_file(&file_path).await
                            .map_err(|e| SelfTestError::Sandbox(format!("Failed to delete file: {}", e)))?;
    ...
    pub async fn run_command(&self, program: &str, args: &[&str]) -> Result<CommandResult, SelfTestError> {
    ...
    pub async fn write_file(&self, relative_path: &Path, content: &str) -> Result<(), SelfTestError> {
        let path = self.get_path(relative_path);
    ...
    async fn copy_directory(&self, from: &Path, to: &Path) -> Result<(), SelfTestError> {
        ...
        while let Some(entry) = entries.next_entry().await
            ...
            if file_name_str == "target" || file_name_str == ".git" || file_name_str == "node_modules" {
                continue;
            }
...
impl ProjectSnapshot {
    ...
    async fn capture_recursive(base: &Path, current: &Path, files: &mut Vec<(PathBuf, Vec<u8>)>) -> Result<(), SelfTestError> {
